...
class TestParseOptionsGracefully:
    """Test cases for the parse_options_gracefully function."""
    ...
    def test_simple_string_format(self):
        ...
        result = parse_options_gracefully(input_str)
    ...
    def test_error_missing_equals(self):
        ...
        with pytest.raises(ValueError, match="missing '='"):
            parse_options_gracefully("LHOST192.168.1.100")

        with pytest.raises(ValueError, match="missing '='"):
            parse_options_gracefully("LHOST=192.168.1.100,LPORT4444")
    ...
    def test_error_empty_key(self):
        ...
        with pytest.raises(ValueError, match="empty key"):
            parse_options_gracefully("LHOST=192.168.1.100,=4444")

    def test_error_invalid_type(self):
        """Test error handling for invalid input types."""
        with pytest.raises(ValueError, match="Options must be a dictionary"):
            parse_options_gracefully(123)

        with pytest.raises(ValueError, match="Options must be a dictionary"):
            parse_options_gracefully([1, 2, 3])

    def test_whitespace_handling(self):
        """Test various whitespace scenarios."""
        # Leading/trailing spaces in whole string
        result = parse_options_gracefully("  LHOST=192.168.1.100,LPORT=4444  ")
        expected = {"LHOST": "192.168.1.100", "LPORT": 4444}
        assert result == expected
    ...
    def test_parametrized_cases(self, input_val, expected):
        ...
        result = parse_options_gracefully(input_val)
        assert result == expected
    ...
    def test_logging_behavior(self):
        ...
        with patch('MetasploitMCP.logger') as mock_logger:
            parse_options_gracefully("LHOST=192.168.1.100,LPORT=4444")
